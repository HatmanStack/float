  ...
  useEffect(() => {
    const processIncidents = async () => {
      ...
      for (const [index, incident] of incidentList.entries()) {
        ...
        const intensity = incident.intensity === '1' ? '2' : incident.intensity;
        const endIndex = Math.min(colorKey + parseInt(intensity) * numberOfColorsToTransitionThrough, colorSets.length);
