...
describe('Error Scenarios', () => {
  ...
  describe('Network Error Scenarios', () => {
    ...
    it('should retry failed summary request', async () => {
      ...
      const summaryVisible = await element(by.id('summary-result')).isVisible();
      const errorVisible = await element(by.id('error-message')).isVisible();
    ...
    it('should handle offline mode gracefully', async () => {
      ...
      await detoxExpect(element(by.id('error-message'))).toHaveText(/offline|network/i);
  ...
  describe('Backend Error Scenarios', () => {
    it('should handle 500 server error during summary', async () => {
      ...
      await detoxExpect(element(by.id('error-message'))).toHaveText(/server|error|500/i);
    ...
    it('should handle invalid response from backend', async () => {
      ...
      await detoxExpect(element(by.id('error-message'))).toHaveText(/invalid|parse|format/i);
    });
  ...
  describe('App State Error Scenarios', () => {
    it('should handle app backgrounding during recording', async () => {
      ...
      const warningVisible = await element(by.id('recording-interrupted-warning')).isVisible();
      const recordingComplete = await element(by.id('processing-indicator')).isVisible();

      if (!warningVisible && !recordingComplete) {
        // At minimum, recording should not still show as active
        const statusText = await element(by.id('recording-status')).getAttributes();
        if (statusText.text === 'Recording...') {
    ...
    it('should handle app crash recovery', async () => {
      ...
      const hasIncidents = await element(by.id('incident-item-0')).isVisible();
      const isEmpty = await element(by.id('empty-history-message')).isVisible();
  ...
  describe('Data Validation Error Scenarios', () => {
    it('should handle too short recording', async () => {
      ...
      await detoxExpect(element(by.id('validation-error'))).toHaveText(/short|minimum|duration/i);
    ...
    it('should handle empty/silent audio recording', async () => {
      ...
      await detoxExpect(element(by.id('error-message'))).toHaveText(
        /silent|no audio|could not detect/i
      );
    });
  });
  ...
  describe('Error Recovery Flows', () => {
    ...
    it('should allow cancelling stuck operation', async () => {
      ...
      if (await element(by.id('cancel-button')).isVisible()) {
